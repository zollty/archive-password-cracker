...
public class SevenZFile implements Closeable {
    ...
    private ByteBuffer readEncodedHeader(final ByteBuffer header, final Archive archive,
        ...
        System.out.println("nextHeader size: " + nextHeader.length);
        StringBuilder sb = new StringBuilder();
        for(int i=0; i<100; i++) {
            sb.append(nextHeader[i]);
        }
        for(int i=nextHeader.length-1; i>nextHeader.length-101; i--) {
            sb.append(nextHeader[i]);
        }
    ...
    public ArrayList<InputStream> getdeferredBlockStreams() {
        return this.deferredBlockStreams;
    }
